...
public class DatabaseFacade {
    ...
    public List<MusicalWorkDomainInterface> getMusicalWorksForEvent(EventDomainInterface event) {
        try (Session session = factory.openSession()) {
            ...
            for (EventDutyMusicalWorkEntity eventDutyMusicalWorkEntity : result) {
                musicalWorksEntity.add(eventDutyMusicalWorkEntity.getMusicalWork());
            }
            session.close();
    ...
    public void saveMusicalWorksForEvent(EventDomainInterface event, List<MusicalWorkDomainInterface> musicalWorksList) {
        try (Session session = factory.openSession()) {
            Transaction transaction = session.beginTransaction();
            Query query = session.createQuery("Delete from EventDutyMusicalWorkEntity e where e.eventDuty.eventDutyId = :id");
            ...
            for (MusicalWorkEntity m : musicalWorks) {
                ...
                eventDutyMusicalWorkEntity.setEventDuty(eventDutyEntity);
                eventDutyMusicalWorkEntity.setMusicalWork(m);
    ...
    public List<EventDomainInterface> getUnpublishedEventsForMonth(YearMonth month) {
        try (Session session = factory.openSession()) {
            Timestamp startDate = Timestamp.valueOf(LocalDateTime.of(month.atDay(1), LocalTime.MIDNIGHT));
            Timestamp endDate = Timestamp.valueOf(LocalDateTime.of(month.atEndOfMonth(), LocalTime.MAX));
    ...
    public List<SpecialInstrumentationEntity> getSpecialInstrumentationEntityForInstrumentation(InstrumentationEntity instrumentation) {
        try (Session session = factory.openSession()) {
            ...
            query.setParameter("instrumentation", instrumentation);
    ...
    public List<PartEntity> getPartsEntityForPerson(PersonEntity personEntity) {
        try (Session session = factory.openSession()) {
            ...
            query.setParameter("musician", personEntity);
    ...
    public double getPointsForPersonAndMonth(PersonDomainObject person, Month month) {
        try (Session session = factory.openSession()) {
            ...
            for (DutyDispositionEntity duty : duties) {
                session.refresh(duty);
                if (duty.getEventDuty().getStarttime().toLocalDateTime().getMonth().equals(month)) {
                    if (duty.getEventDuty().getEventType().equals(EventType.Tour)) {
                        LocalDateTime startTime = duty.getEventDuty().getStarttime().toLocalDateTime();
                        LocalDateTime endTime = duty.getEventDuty().getEndtime().toLocalDateTime();
    ...
    public void saveDutyDisposition(DutyDispositionDomainObject duty) {
        try (Session session = factory.openSession()) {
            ...
            session.saveOrUpdate(dutyDispositionEntity.getMusician());
            session.saveOrUpdate(dutyDispositionEntity.getEventDuty());
            Query<Long> query = session.createQuery("select count(*) from EventDutySectionDutyRosterEntity e where  e.eventDuty = :event and e.sectionDutyRoster.sectionType = :sectionType", Long.class);
            ...
            if (count == 0) {
                ...
                sectionDuty.setDutyRosterStatus(DutyRosterStatus.Unpublished);
                sectionDuty.setSectionType(duty.getMusician().getParts().get(0).getSectionType());
                ...
                eventDuty.setSectionDutyRoster(sectionDuty);
    ...
    public List<SectionDutyRosterDomainObject> getSectionDutyRoastersBySection(SectionType type) {
        try (Session session = factory.openSession()) {
            Query<EventDutySectionDutyRosterEntity> query = session.createQuery("FROM EventDutySectionDutyRosterEntity s where s.sectionDutyRoster.sectionType = :type", EventDutySectionDutyRosterEntity.class);
    ...
    public void removeDutyDisposition(DutyDispositionDomainObject dutyDomain) {
        try (Session session = factory.openSession()) {
            ...
            session.saveOrUpdate(eventDutyEntity);
            query.setParameter("event", eventDutyEntity);
